...
const Product: React.FC<{}> = () => {
  ...
  const { _addProductToCartAction } = bindActionCreators(
    cartActionCreators,
    dispatch
  );

  useEffect(() => {
    const productDidMount = async () => {
      const getId = getParamsId ?? "xAerCF123123";
      fetchProductDetail(getId, _resolveAddProductToDetail, _rejectAddProductToDetail);
    };
    productDidMount()
  }, []);
  ...
  const handleSubmit = (evt: { preventDefault: () => void; }) => {
    evt.preventDefault();
    if (selectedSize === "") return setWarningSelectedSize(true);
    const product = products.detail.data;
    _addProductToCartAction({
      ...
      size: selectedSize,
      color: selectedColor ?? "",
      imageSrc: product?.images.item ?? "",
      imageAlt: product?.images.alt ?? "",
      quantity: 1,
      href: `stores/${name}?id=${getParamsId}&color=${selectedColor}&size=${selectedSize}`
    })
  }
  ...
  return (
    <div className="bg-white">
      <div className="pt-6">
        ...
        <Breadcrumbs />
        {
          products.detail.loading
            ? <FallbackLoading message='Loading..' minHeight={"600px"} />
            : !products.detail.loading && products.detail.data === null
              ? <FallbackLoading message={TestId.containers.product.productNotFound} minHeight={"600px"} />
              : <div>
                {/* Image gallery */}
                ...
                <div className="max-w-2xl mx-auto pt-10 pb-16 px-4 sm:px-6 lg:max-w-7xl lg:pt-16 lg:pb-24 lg:px-8 lg:grid lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8">
                  ...
                  <div className="mt-4 lg:mt-0 lg:row-span-3">
                    ...
                    >
                          ...
                          <h3 className="text-sm text-gray-900 font-medium required">Size</h3>
                          {
                            warningSelectedSize && (
                              <h3 className="text-sm font-medium text-amber-600">
                                {TestId.button.container.product.sizeValidationMessage}
                              </h3>
                            )
                          }
                        </div>
                      ...
                      <button
                        data-testid={TestId.button.container.product.add_cart_button}
                        type="submit"
                        value="Submit"
                        className="mt-10 w-full bg-indigo-600 border border-transparent rounded-md py-3 px-8 flex items-center justify-center text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                      >
                        {TestId.button.container.product.add_cart_button_title}
                      </button>
                    </form>
                  </div>
